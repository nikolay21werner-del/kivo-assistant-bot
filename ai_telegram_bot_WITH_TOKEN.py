...
class AdvancedTelegramAgent:
    KEYWORDS = {
        IntentType.ORDER: ["заказ", "купить", "бронь", "бронировать", "доставка", "цена", "стоимость", "прайс", "заказать"],
        IntentType.SUPPORT: ["помощь", "проблема", "ошибка", "не работает", "баг", "support", "помогите", "сбой", "технич"],
        IntentType.NOTIFICATION: ["уведомл", "напомни", "оповести", "отправь", "рассылка"],
        IntentType.DATABASE: ["сохрани", "запомни", "добавь", "удали", "профиль", "данные", "информацию"]
    }
    ...
    async def _process_order(self, user_id: str, message: str):
        ...
        self.orders[order_id] = Order(order_id=order_id, user_id=user_id, description=message)
    ...
    async def _create_ticket(self, user_id: str, issue: str):
        ...
        self.tickets[ticket_id] = SupportTicket(ticket_id=ticket_id, user_id=user_id, issue=issue)
    ...
    async def handle_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.message:
            await update.message.reply_text(f"📊 Заказов: {len(self.orders)}, Тикетов: {len(self.tickets)}")
